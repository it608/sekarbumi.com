<?php
if ( ! defined( 'ABSPATH' ) ) exit;
?>
<div class="ycf-bootstrap-wrapper ypm-support-metabox">
	<h3>Support <span class="dashicons dashicons-megaphone"></span></h3>
	<p>
		We love our plugin and do the best to improve all features for You. But sometimes issues happened, or you can't find required feature that you need. Don't worry, just  pressing here
		<br>
		<div style="text-align: center">
			<a href="<?php echo YPM_SUPPORT_URL; ?>" style="font-size: 18px; cursor: pointer;" target="_blank">
				<button type="button" id="ypm-report-problem-button" class="ypm-support-button-red" style="margin: 10px;">
					<i class="ai1wm-icon-notification"></i>
					Report issue
				</button>
			</a>
		</div>
		and we will help you!</p>
</div>
<style>
	.ypm-support-metabox {
		text-align: left !important;
	}
</style>