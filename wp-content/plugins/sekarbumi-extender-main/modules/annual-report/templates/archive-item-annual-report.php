<?php
$skbm_annual_report_link  = get_post_meta(get_the_ID(), 'skbm_annual_report_link', true);
    ...
    <div class="skbm-cpt-card__inner">
      <span class="skbm-cpt-card__misc">
        ...
        if ($current_language == 'id_ID') {
          printf('%s', __('LAPORAN TAHUNAN', 'skbm-extender'));
        } else printf('%s', __('ANNUAL REPORT', 'skbm-extender'))
        ?>
      </span>
      <h1 class="skbm-cpt-card__post-title uk-margin-bottom"><?php the_title(); ?></h1>
      <a href="<?php echo !empty($skbm_annual_report_link) ? esc_url($skbm_annual_report_link) : ''; ?>" target="_blank" class="skbm-cpt-card__button uk-border-rounded">
        <?php
        if ($current_language == 'id_ID') {
