<?php
$skbm_press_report_link  = get_post_meta(get_the_ID(), 'skbm_press_report_link', true);
  ...
  <div class="skbm-cpt-card uk-box-shadow-medium uk-box-shadow-hover-large uk-border-rounded uk-overflow-hidden uk-text-center" data-uk-scrollspy="cls:uk-animation-slide-bottom">
    <span class="skbm-cpt-card__misc">
      ...
      if ($current_language == 'id_ID') {
        printf('%s', __('SIARAN PERS', 'skbm-extender'));
      } else printf('%s', __('PRESS RELEASE', 'skbm-extender'))
      ?>
    </span>
    <h1 class="skbm-cpt-card__post-title uk-margin-bottom"><?php the_title(); ?></h1>
    <a href="<?php echo !empty($skbm_press_report_link) ? esc_url($skbm_press_report_link) : ''; ?>" target="_blank" class="skbm-cpt-card__button uk-border-rounded">
      <?php
      if ($current_language == 'id_ID') {
